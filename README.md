...
2. Its search bar includes a button that opens a map on which users can draw points, lines, and shapes in different colors along with their text search query. Users can choose to view search results on a map, with the properties and businesses from the results plotted on it.
...
9. It delivers significantly more relevant results by proposing a better architecture that addresses the limitations of embedding based candidate retrieval (dense retrieval), ranking models etc. It does use embedding models, ANN and ranking algorithms being used by current SOTA search pipelines as it's impossible to use LLMs to scan all images and read product description of all products to answer a query.
